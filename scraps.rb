...
require "stripe"
STRIPE_PUBLIC_KEY = ENV['FRETBUZZ_STRIPE_PK']
STRIPE_PRIVATE_KEY = ENV['FRETBUZZ_STRIPE_SK']
Stripe.api_key = STRIPE_PRIVATE_KEY

StripeEvent.authentication_secret = ENV['FRETBUZZ_STRIPE_WHS']

# Receives web hook from Stripe, uses that info to call back to 
# Stripe to validate authenticity of request, then processes.
#
# Event subscriptions only fire on successful validation to prevent
# illegitimate requests from triggering actions in the application.
...
# progress bar - dots and percent, wrapping

if index % 100 == 0
  percent = (index / total.to_f) * 100
  hitrate = good == 0 ? 0.0 : (good / index.to_f) * 100 
  puts " - #{index} of #{total} (#{sprintf('%.2f%', percent)}) +#{good}/-#{bads} : #{sprintf('%.2f%', hitrate) }%"
end
